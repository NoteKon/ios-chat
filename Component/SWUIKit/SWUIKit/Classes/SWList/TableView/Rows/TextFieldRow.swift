...
// MARK:- TextFieldCell
open class TextFieldCell: SWTableCellOf<String> {
...
// MARK:- TextFieldRow
open class _TextFieldRowOf<T: TextFieldCell>: SWTableRowOf<T>, UITextFieldDelegate {
    ...
    // 设置回调事件
    public func onTextShouldChange(_ callBack:@escaping ((_ row: _TextFieldRowOf<T>, _ textField: UITextField, _ range: NSRange, _ string: String) -> Bool)) {
        onTextShouldChangeBlock = { [weak self](textField, range, string) in
            return callBack(self!, textField, range, string)
        }
    }
    public func onTextDidChanged(_ callBack: @escaping ((_ row: _TextFieldRowOf<T>, _ textField: UITextField) -> Void)) {
        onTextDidChangeBlock = { [weak self] (textField) in
            callBack(self!, textField)
        }
    }
    public func onTextFieldDidBeginEditing(_ callBack: @escaping ((_ row: _TextFieldRowOf<T>, _ textField: UITextField) -> Void)) {
        onTextFieldDidBeginEditingBlock = { [weak self] (textField) in
            callBack(self!, textField)
        }
    }
    public func onTextFieldDidEndEditing(_ callBack: @escaping ((_ row: _TextFieldRowOf<T>, _ textField: UITextField) -> Void)) {
        onTextFieldDidEndEditingBlock = { [weak self] (textField) in
            callBack(self!, textField)
        }
    }
    public func onTextFieldShouldReturn(_ callBack: @escaping ((_ row: _TextFieldRowOf<T>, _ textField: UITextField) -> Bool)) {
        onTextFieldShouldReturnBlock = { [weak self] (textField) in
            return callBack(self!, textField)
        }
    }
    public func onTextFieldShouldClear(_ callBack: @escaping ((_ row: _TextFieldRowOf<T>, _ textField: UITextField) -> Bool)) {
...
///  带textfield的Row，可展示标题和输入框，同时提供自定义标题、输入框样式等属性
public final class TextFieldRow: _TextFieldRowOf<TextFieldCell>, SWRowType {
}
